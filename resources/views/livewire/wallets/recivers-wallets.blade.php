    ...
    <div class="bg-white dark:bg-zinc-800  relative  overflow-hidden">
        <div class="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
            ...
            <div class="w-full flex flex-row md:flex-row md:items-center md:justify-end gap-2 md:gap-3">
                <flux:modal.trigger name="Create-wallet-modal">
                    <!-- زر إنشاء دور -->
                ...
                <flux:modal name="Create-wallet-modal" class="md:w-96">
                    <div class="space-y-6">
                        <div>
                            <flux:heading size="lg">{{ __('Create New') }}</flux:heading>
                        </div>
                        <flux:input wire:model.live='walletName' label="{{ __('wallet Name') }}"
                            placeholder="{{ __('wallet Name') }}" />
                        <flux:input wire:model.live='walletAddress' label="{{ __('wallet Address') }}"
                            placeholder="{{ __('wallet Adress') }}" />

                        <flux:fieldset>
                            <flux:legend>{{ __(' Status') }}</flux:legend>
                            <div class="space-y-3">
                                <flux:switch wire:model.live='walletStatus' label="{{ __('Wallet Status') }}" />

                            </div>
                        </flux:fieldset>
                        <div class="flex">
                            <flux:spacer />
                            <flux:button wire:click="createWallet" type="submit" variant="primary">{{ __('Create') }}
                            </flux:button>
                        </div>
                    </div>
                </flux:modal>
                ...
                <flux:modal name="edit-wallet-modal" class="md:w-96">
                    <div class="space-y-6">
                        <div>
                            <flux:heading size="lg">{{ __('Create New') }}</flux:heading>
                        </div>
                        ...
                        <div class="flex">
                            <flux:spacer />
                            <flux:button wire:click="EditWallet" type="submit" variant="primary">{{ __('Edit') }}
                            </flux:button>
                        </div>
                    </div>
                </flux:modal>
            ...
            <table class="w-full text-sm text-center text-gray-500 dark:text-zinc-400 dark:border-gray-700 rounded-md">
                <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-zinc-700  dark:text-zinc-400">
                    <tr>
                        ...
                        <th scope="col" class="px-4 py-3">{{ __('Adress') }}</th>
                ...
                <tbody>
                    @forelse ($wallets as $wallet)
                        <tr class="border-b dark:border-gray-700">
                            ...
                            <td class="px-4 py-3">{{ $wallet->address }}</td>
                            ...
                            <td class="px-4 py-3 flex items-center justify-center">
                                <div class="flex flex-row items-center justify-center gap-2">
                                    ...
                                    <a href="javascript:void(0)" wire:click="openEditModal('{{ $wallet->id }}')"
                                    ...
                                    <a href="javascript:void(0)" wire:click="delete('{{ $wallet->id }}')"
                                        class="flex items-center justify-center text-xs
