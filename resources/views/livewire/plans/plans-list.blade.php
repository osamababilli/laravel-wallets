<div>
    ...
    <div class="space-y-2 mb-5">
        ...
        <flux:subheading>
            {{ __('Manage your investment plans') }}
        </flux:subheading>
    ...
    <div class="bg-white dark:bg-zinc-800  relative  overflow-hidden">
        <div class="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
            ...
            <flux:modal name="NewPlan" class="md:w-1/2">
                <div class="space-y-6">
                    <div>
                        <flux:heading size="lg">{{ __('New Plan Details') }}</flux:heading>
                    </div>
                    <flux:input wire:model.live='name' type="text" label="{{ __('Name') }} " />

                    <flux:input wire:model.live='requiredAmount' type="number" label="{{ __('Amount') }}" />

                    <flux:input wire:model.live='profit' type="number" label="{{ __('Profit') }}" />


                    <flux:select wire:model.live="type" :label="__('Profit Type')">
                    ...
                    <div class="flex">
                        <flux:spacer />
                        <flux:button wire:click="CreatePlanFn" type="submit" variant="primary">{{ __('submit') }}
                        </flux:button>
                    </div>
                </div>
            </flux:modal>
            ...
            <flux:modal name="editPlan" class="md:w-1/2">
                <div class="space-y-6">
                    <div>
                        <flux:heading size="lg">{{ __('Edit Plan Details') }}</flux:heading>
                    </div>
                    <flux:input wire:model.live='name' type="text" label="{{ __('Name') }} " />
                    ...
                    <flux:field>
                        <flux:label>{{ __('Plan Status') }}</flux:label>

                        <flux:switch wire:model.live="status" :checked="$status === 'active'" />
                    ...
                    <div class="flex">
                        <flux:spacer />
                        <flux:button wire:click="editPlanFn" type="submit" variant="primary">{{ __('submit') }}
                        </flux:button>
                    </div>
                </div>
            </flux:modal>
            ...
            <div class="w-full flex flex-row md:flex-row md:items-center md:justify-end gap-2 md:gap-3">
                ...
                <div class="w-full md:w-auto">
                    <a href="javascript:void(0)" x-on:click="$flux.modal('NewPlan').show()"
                  ...
                  focus:outline-none focus:ring-2 focus:ring-zinc-300">
                        <svg class="h-4 w-4" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"
                            aria-hidden="true">
                            <path clip-rule="evenodd" fill-rule="evenodd" d=" M10 3a1 1 0 011 1v5h5a1 1 0 110 2h-5v5a1
                        1 0 11-2 0v-5H4a1 1 0 110-2h5V4a1 1 0 011-1z" />
                        </svg>
                        {{ __('Create New Plan') }}
                    </a>
                </div>
            ...
            <table class="w-full text-sm text-center text-gray-500 dark:text-zinc-400 dark:border-gray-700 rounded-md">
                <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-zinc-700  dark:text-zinc-400">
                    <tr>
                        ...
                        <th scope="col" class="px-4 py-3 text-center">{{ __('Profit Type') }}</th>
                        <th scope="col" class="px-4 py-3 text-center">{{ __('Plan Members') }}</th>
                ...
                <tbody>
                    @forelse ($plans as $plan)
                        <tr class="border-b dark:border-gray-700">
                            ...
                            <td class="px-4 py-3">{{ $plan->amount }} USDT</td>
                            <td class="px-4 py-3">{{ $plan->profit }} USDT </td>
                            <td class="px-4 py-3">{{ \Str::ucfirst($plan->type) }}</td>
                            <td class="px-4 py-3">{{ $plan->members()->count() }}</td>
                            ...
                            <td class="px-4 py-3 flex items-center justify-center">
                                <div class="flex flex-row items-center justify-center gap-2">
                                    ...
                                    <a href="javascript:void(0)" wire:click="EditPlan('{{ $plan->id }}')"
                                    ...
                                    <a href="javascript:void(0)" wire:click="delete('{{ $plan->id }}')"
                                        class="flex items-center justify-center text-xs
